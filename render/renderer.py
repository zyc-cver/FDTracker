...
class Renderer:
    ...
    def create_camera(self, R=None, T=None):
        ...
        return PerspectiveCameras(
            device=self.device, R=self.R.transpose(-1, -2), T=self.T, K=self.K_full, image_size=self.image_sizes, in_ndc=False
        )
    ...
    def initialize_camera_params(self, focal_length, K):
        ...
        if K is not None:
            if isinstance(K, np.ndarray):
                self.K = torch.from_numpy(K).float().reshape(1, 3, 3).to(self.device)
            else:
    ...
    def set_ground(self, length, center_x, center_z):
        ...
        v, f, vc, fc = map(torch.from_numpy, checkerboard_geometry(
            length=length, 
            c1=center_x, 
            c2=center_z, 
            up="y",
    ...
    def render_mesh(self, vertices, background=None, colors=[0.0, 0.0, 0.5], VI=50):
    ...
    def render_with_ground(self, verts, colors, cameras, lights, faces=None):
        ...
        verts = [v.contiguous() for v in verts]
        faces = [f.contiguous() for f in faces]
        colors = [c.contiguous() for c in colors]
        ...
        try:
            ...
            if "out of memory" in str(e):
                ...
                print("GPU memory cleared, retrying render...")
                results = self.renderer(mesh, cameras=cameras, lights=lights, materials=materials)
                image = (results[0, ..., :3].cpu().numpy() * 255).astype(np.uint8)
            else:
                raise e
...
def get_global_cameras(verts, device="cuda", distance=5, position=(-5.0, 5.0, 0.0)):
    ...
    rotation = look_at_rotation(positions, targets).transpose(-1, -2)
...
def get_global_cameras_static(
    ...
    rotation = look_at_rotation(positions, target_centers).transpose(-1, -2)
    translation = -(rotation @ positions.unsqueeze(-1)).squeeze(-1)
