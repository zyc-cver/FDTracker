...
from lib.utils.funcs_utils import transform_joint_to_other_db, scipy_to_pytorch, adjmat_sparse, spmm
...
class SMPLH(object):
    ...
    def get_coords(self, smpl_pose, smpl_shape, gender='neutral'):
        smpl_shape = torch.tensor(smpl_shape.reshape(-1, 10), dtype=torch.float32)
        smpl_pose = torch.tensor(smpl_pose.reshape(-1, 156), dtype=torch.float32)
        ...
        smpl_joint_cam = torch.matmul(torch.tensor(self.joint_regressor[None,:,:], dtype=torch.float32), smpl_mesh_cam)
