...
from utils.eval_lib import metrics as metric
from utils.eval_lib.chamfer_distance import chamfer_distance
from utils.evaluate_human import HumanEvaluator
import utils.eval_lib.config as config
...
class JointReconEvaluator(HumanEvaluator):
    def eval(self, res_dir, gt_dir, outfile):
        ...
        if data_complete:
            error_dict, videos_error = self.compute_errors(data_gt, data_pred)
        else:
            error_dict = {}
