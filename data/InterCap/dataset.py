...
from data_utils import trans_obj_id
...
class InterCap(Dataset):
    ...
    def _load_data(self, idx):
        ...
        # data = self.motion_files[vid_name]
...
class InterCapTest(Dataset):
    ...
    def _load_dataset(self):
        # Load input data (features, etc.)
        input_data_path = osp.join(self.data_path, 'hum_feat.pt')
        bbox_hum_path = osp.join(self.data_path, 'hum_bbox.pt')
        
        if not osp.exists(input_data_path):
            print(f"Warning: Input data file not found at {input_data_path}")
            return
